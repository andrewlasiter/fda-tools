...
Usage:
from fda_tools.lib.rbac import require_role, require_permission, check_permission
from fda_tools.lib.auth import Role, User
...
from fda_tools.lib.auth import AuthManager, Role, User, AuditEventType
...
# ============================================================
# Session Token Validation (FDA-208 / SEC-016)
# ============================================================

def is_valid_session_token_format(token: str) -> bool:
    """Validate that a string has valid session token format.

    Session tokens are generated by secrets.token_hex(64), producing
    128-character hex strings. This function checks format only, not
    whether the token exists in the session store.

    Args:
        token: String to validate

    Returns:
        True if token has valid format (128 hex chars), False otherwise

    Examples:
        >>> is_valid_session_token_format("a" * 128)  # All hex
        True
        >>> is_valid_session_token_format("Z" * 128)  # Not hex
        False
        >>> is_valid_session_token_format("a" * 64)  # Wrong length
        False
        >>> is_valid_session_token_format("")
        False

    Note:
        This fixes FDA-208 (SEC-016) - prevents arbitrary 128-char strings
        from being misidentified as session tokens.
    """
    if not isinstance(token, str):
        return False

    # Must be exactly 128 characters (64 bytes hex-encoded)
    if len(token) != 128:
        return False
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            if not session_token and args:
                ...
                # FDA-208 (SEC-016): Validate token format to prevent misidentification
                if is_valid_session_token_format(args[0]):
