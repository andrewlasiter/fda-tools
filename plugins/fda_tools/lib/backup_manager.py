...
from fda_tools.lib.subprocess_helpers import SubprocessError, run_command

# Commands allowed by this module's backup operations (FDA-129)
_BACKUP_ALLOWLIST = ["docker", "docker-compose", "gpg", "aws", "createdb", "dropdb", "psql"]
...
class BackupManager:
    ...
    def create_full_backup(self, compress: bool = True) -> Path:
        ...
        result = run_command(cmd, allowlist=_BACKUP_ALLOWLIST)
        ...
        result = run_command(
            ...
            allowlist=_BACKUP_ALLOWLIST,
        )
        if result.returncode != 0:
            raise BackupError(f"Failed to copy backup from container: {result.stderr}")
    ...
    def encrypt_backup(self, backup_file: Path, recipient: str) -> Path:
        ...
        if result.returncode != 0:
            raise BackupError(f"GPG encryption failed: {result.stderr}")
    ...
    def decrypt_backup(self, encrypted_file: Path, output_file: Optional[Path] = None) -> Path:
        ...
        if result.returncode != 0:
            raise RestoreError(f"GPG decryption failed: {result.stderr}")
    ...
    def restore_backup(self, backup_file: Path, database: Optional[str] = None) -> bool:
        ...
        if result.returncode != 0:
            raise RestoreError(f"Failed to copy backup to container: {result.stderr}")
    ...
    def verify_backup(self, backup_file: Path) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            if result.returncode != 0:
                raise BackupError(f"Failed to create test database: {result.stderr}")
...
"""
        ...
        if result.returncode != 0:
            logger.error(f"Failed to configure WAL archiving: {result.stderr}")
            return False
